...
export default function SolutionStep({ data, onChange }: Props) {
  ...
  const update = (idx: number, val: string) => {
    const next = [...points];
    next[idx] = val;
    onChange({ ...data, points: next });
  };

  const add = () => {
    if (points.length >= 10) return;
    onChange({ ...data, points: [...points, ""] });
  };

  const remove = (idx: number) => {
    onChange({ ...data, points: points.filter((_, i) => i !== idx) });
  };
    ...
    <div className="space-y-5">
      ...
      <label className="block mt-4">
        <span className="text-sm font-medium text-gray-700">
          Additional Statement (bottom-left of slide)
        </span>
        <input
          type="text"
          ...
          placeholder="e.g. Our solution is designed to scale with your growth."
        />
      </label>
    </div>
  );
